...
DELTA_D = 1 # delta d in lateral lattice
DELTA_S = 1 # delta s in longitudinal lattice
...
class frenet_trajectory_sets(): #a set of frenet trajectories within a planning cycle
    ...
    def gen_lat_lattice(self,d0,d_d0,dd_d0,target_path):
        ...
        t_horizon = np.arange(self.t0,self.t1, DELTA_T) + DELTA_T #normalie time tstart at 0+DELTA_T
        for t in t_horizon:
            d_horizon = np.arange(target_path.lower_eval(t),target_path.upper_eval(t) + DELTA_D,DELTA_D)
            sample_pts = np.arange(self.t0,t,1/T_RES)
            for d in d_horizon:
                target_d = target_path.traj.eval(t)
                d_target_d = target_path.traj.first_derivative_eval(t)
                dd_target_d = target_path.traj.second_derivative_eval(t)
                traj = quint_poly(d0,d_d0,dd_d0,d,d_target_d,dd_target_d, t-self.t0)
    ...
    def cruising_lattice(self,s0,d_s0,dd_s0,target_path): #target_path = dST_path
        self.lon_lattice = []
        self.min_lon_cost = math.inf
        t_horizon = np.arange(self.t0, self.t1, DELTA_T) + DELTA_T
        for t in t_horizon:
...
class quad_poly():
    ...
    def plot(self,ax,col,w):
        line = ax.plot(self.t_pts,self.traj_d_eval,color = col, lw = w)
        return line
...
class SL_path():    #LatOffset_station path
    def __init__(self,target,t_begin,t_end,width):
        self.traj = target
        self.begin = t_begin
        self.end = t_end
...
class simulation():
    def __init__(self):
        ...
        #lon_line = cubic_poly(5,0,0,0) # station - time plot                   '''longitudinal(s) error to time path setting'''
        #self.lon_target_path = ST_path(lon_line,0,SIM_TIME,DS_MARGIN)
        cruise_dST_plt = cubic_poly(8,0,0,0)
        self.lon_target_path = dST_path(target = cruise_dST_plt, t_begin = 0, t_end = SIM_TIME, min = 0, max = SPEED_LIMIT)

    def next(self):
        self.t = self.t + SIM_STEP

    def update_pos(self):
        self.d0 = self.fts.lat_opt_traj.eval(self.t-self.planing_start_t)[0]
        self.d_d0 = self.fts.lat_opt_traj.first_derivative_eval(self.t-self.planing_start_t)[0]
        self.dd_d0 = self.fts.lat_opt_traj.second_derivative_eval(self.t-self.planing_start_t)[0]
        self.s0 = self.fts.lon_opt_traj.eval(self.t-self.planing_start_t)[0]
        self.d_s0 = self.fts.lon_opt_traj.first_derivative_eval(self.t-self.planing_start_t)[0]
        self.dd_s0 = self.fts.lon_opt_traj.second_derivative_eval(self.t-self.planing_start_t)[0]

    def plan(self):
        self.planing_start_t = self.t
        self.fts = frenet_trajectory_sets([self.planing_start_t,self.planing_start_t + P_T])
        ...
        self.fts.cruising_lattice(self.s0,self.d_s0,self.dd_s0,self.lon_target_path)
        self.fts.plot_lon_lattice(self.ax2[0])
        print('*************time:',self.t, self.fts.lon_opt_traj.traj_d_eval,'\n opt_traj:', self.fts.lon_opt_traj.a2)
    ...
    def init_long_lat_plot(self):
        ...
        self.line_lon, = self.ax2[0].plot([],[],color = 'green',zorder = math.inf)
        self.line_lat, = self.ax2[1].plot([],[],color = 'green',zorder = math.inf)
    ...
    def refresh_plots(self):
        ...
        self.lon_ydata.append(self.d_s0)
        self.lat_ydata.append(self.d0)
        self.line_lon.set_data(self.lon_xdata,self.lon_ydata)
        self.line_lat.set_data(self.lat_xdata,self.lat_ydata)
